...
class Body {
    constructor(entries) {
        ...
        __classPrivateFieldSet(this, _Body_entries, entries.map(([path, value]) => [
            ...
            isFile_1.default(value) || isField_1.default(value)
                ? value
                : new BodyField_1.BodyField(value, pathToFieldname_1.default(path))
        ]), "f");
    }
    /**
    ...
    *values() {
    ...
    json() {
        const entries = [];
        for (const [path, value] of this) {
            entries.push([path, isFile_1.default(value) ? value : value.valueOf()]);
        }
        return object_deep_from_entries_1.default(entries);
    }
    /**
    ...
    formData() {
        ...
        for (const [path, value] of this) {
            form.append(pathToFieldname_1.default(path), isFile_1.default(value) ? value : value.valueOf());
        }
        return form;
    }
}
